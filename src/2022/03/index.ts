 ...
 */
export function solvePart1(input: Day3Input) {
  return input.reduce((total, prios) => {
    const first = prios.slice(0, prios.length / 2);
    const second = prios.slice(prios.length / 2);
    return total + first.find((prio) => second.includes(prio))!;
  }, 0);
}
 ...
 * this must be their badges. In the second group, 
 ...
 * What is the sum of the priorities of those item types?
 */
export function solvePart2(input: Day3Input) {
  const groups: number[][][] = [];
  const groupSize = 3;
  ...
  return groups.reduce(
    (total, group) => {
      return total + group[0].filter((prio) => {
        return group.every((group) => {
          return group.indexOf(prio) >= 0;
        });
      })[0];
    },
    0,
...
export function parseInputFile(filePath: string): Day3Input {
  const text = readFileSync(filePath).toString();
  const lines = text.split('\r\n');
  const result = lines.map((line): number[] => {
    // const first = line.substring(0, line.length / 2);
    // const second = line.substring(line.length / 2);
    return [...line].map((char) => {
      return 1 + 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'.indexOf(char);
    });
  });
