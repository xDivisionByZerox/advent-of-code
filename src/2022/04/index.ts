...
export type Day4Input = [[number, number], [number, number]][];
 ...
 */
export function solvePart1(input: Day4Input) {
  let count = 0;
 ...
 */
export function solvePart2(input: Day4Input) {
  let count = 0;
  for (const pairs of input) {
    const [[min1, max1], [min2, max2]] = pairs;
...
export function parseInputFile(filePath: string): Day4Input {
  const text = readFileSync(filePath).toString();
  const lines = text.split('\n');
  const pairs = lines.map((line): [[number, number], [number, number]] => {
    const [pair1, pair2] = (line.split(',') as [string, string]).map((elem): [number, number] => {
