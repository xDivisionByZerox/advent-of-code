 ...
 */
export function solvePart1(input: Day1Input) {
  return input.sort((prev, curr) => prev - curr)[input.length - 1];
}
 ...
 */
export function solvePart2(input: Day1Input) {
  input.sort((prev, curr) => prev - curr);

  return input.slice(-3).reduce((prev, curr) => prev + curr, 0);
}

export function parseInputFile(inputPath: string): Day1Input {
  const text = readFileSync(inputPath).toString();
  ...
  for (const line of lines) {
    if (line === '') {
      elves.push(currentElve);
      currentElve = 0;
      continue;
    }

    currentElve += parseInt(line, 10);
  }
