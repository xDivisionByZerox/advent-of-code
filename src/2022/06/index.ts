 ...
 */
export function solvePart1(input: Day6Input): number {
  const blockLength = 4;
  for (let i = blockLength; i < input.length; i++) {
    const chars = input.substring(i - blockLength, i);
    const uniqueChars = new Set(chars);
    if (uniqueChars.size === blockLength) {
      return i;
    }
  }
 ...
 * - `W`: first marker after character 26
 *
 * How many characters need to be processed before the first start-of-message marker is detected?
 */
export function solvePart2(input: Day6Input): number {
...
export function parseInputFile(filePath: string): Day6Input {
