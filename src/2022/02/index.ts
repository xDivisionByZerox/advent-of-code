...
export type Method = 'rock' | 'paper' | 'scissors';

export type MeText = 'X' | 'Y' | 'Z';

export type Day2Input = [Method, MeText][];
 ...
 */
export function solvePart1(input: Day2Input) {
  let totalScore = 0;
  for (const round of input) {
    const [enemy, meText] = round;
    const me = ((): Method => {
      switch (meText) {
        case 'X':
          return 'rock';
    ...
    const methodScore = getScoreFromMethod(me);

    const vsScore = (() => {
      if (enemy === me) {
        return 3;
      } else if (
        (enemy === 'paper' && me === 'scissors') ||
        (enemy === 'rock' && me === 'paper') ||
        (enemy === 'scissors' && me === 'rock')
      ) {
        return 6;
      } else {
        return 0;
    ...
    const roundScore = methodScore + vsScore;
 ...
 * what would your total score be if everything goes exactly according to your strategy guide?
 */
export function solvePart2(input: Day2Input) {
  let totalScore = 0;
  for (const round of input) {
    const [enemy, meText] = round;

    const vsScore = ((): number => {
      switch (meText) {
        case 'X':
          return 0;
        case 'Y':
    ...
    const methodScore = (() => {
      if (meText === 'X') {
        if (enemy === 'paper') {
          return getScoreFromMethod('rock');
        } else if (enemy === 'rock') {
          return getScoreFromMethod('scissors');
        } else {
          return getScoreFromMethod('paper');
        }
      } else if (meText === 'Z') {
        if (enemy === 'paper') {
          return getScoreFromMethod('scissors');
        ...
        return getScoreFromMethod(enemy);
      }
    })();
...
function getScoreFromMethod(method: Method): 1 | 2 | 3 {
  switch (method) {
    case 'rock':
      return 1;
    case 'paper':
...
export function parseInputFile(filePath: string): Day2Input {
  const text = readFileSync(filePath).toString();
  const lines = text.split('\r\n');
  const strategy = lines.map((line): [Method, MeText] => {
    const [enemyText, myText] = line.split(' ');
    const getEnemyMethod = (text: string): Method => {
      switch (text) {
        case 'A':
          return 'rock';
        case 'B':
        ...
        case 'C':
          return 'scissors';
        default:
          throw new Error(`unknown enemy strategy "${text}".`);
      }
    };
    const getMyMethod = (text: string): MeText => {
      switch (text) {
        case 'X':
          return 'X';
        case 'Y':
        ...
        case 'Z':
          return 'Z';
        default:
          throw new Error(`unknown strategy for me "${text}".`);
      }
    };

    return [getEnemyMethod(enemyText), getMyMethod(myText)];
  });
