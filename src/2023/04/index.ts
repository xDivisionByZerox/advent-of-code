...
export class Solution04 extends AbstractSolution<number> {
  ...
  protected implementPart1(input: string[]): number {
    return input.reduce<number>((result, line) => {
      const [firstPart, secondPart] = line.split(':')[1].split('|');
      const winningNumbers = Array.from(firstPart.matchAll(/\d+/g)).map((match) => Number.parseInt(match[0], 10));
      const playingNumbers = Array.from(secondPart.matchAll(/\d+/g)).map((match) => Number.parseInt(match[0], 10));

      const winners = playingNumbers.filter((number) => winningNumbers.includes(number));
      ...
      if (winners.length <= 0) {
        ...
        return result + Math.pow(2, winners.length - 1);
      }
    }, 0);
  }
