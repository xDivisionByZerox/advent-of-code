...
export class Solution03 extends AbstractSolution<number> {
  ...
  protected implementPart1(input: string[]): number {
    ...
    for (let rowIndex = 0; rowIndex < input.length; rowIndex++) {
      ...
      for (const numberMatch of numberMatches) {
        ...
        const endIndex = startIndex + numberAsString.length;
        const neighbours = this.getAdjacentCells({
          grid: input,
          currentRow: rowIndex,
          numberStartIndex: startIndex,
          numberEndIndex: endIndex,
        });
        const isAdjacentToSymbol = neighbours.some((cell) => this.isSymbol(cell));
   ...
   * Maybe something is still wrong? Fortunately,
   * the gondola has a phone labeled "help",
  ...
  private getAdjacentCells(options: {
    grid: string[];
    numberStartIndex: number;
    numberEndIndex: number;
    currentRow: number;
  }): string[] {
    const { grid, numberStartIndex, numberEndIndex, currentRow } = options;
    const neighbours: string[] = [];

    for (let rowIndex = currentRow - 1; rowIndex <= currentRow + 1; rowIndex++) {
      const row = grid[rowIndex];
      if (row === undefined) {
        continue;
      }

      for (let colIndex = numberStartIndex - 1; colIndex <= numberEndIndex; colIndex++) {
