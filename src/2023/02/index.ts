...
export class Solution02 extends AbstractSolution<number> {
  ...
  protected implementPart1(input: string[]): number {
    ...
    return input.reduce((result, line): number => {
      const game = this.parseLine(line);
      const invalidEntry = game.rounds.find(({ blue, green, red }) => {
        return blue > allowedBlue || green > allowedGreen || red > allowedRed;
      });

      return invalidEntry !== undefined ? result : result + game.id;
    }, 0);
  }
  ...
  protected implementPart2(input: string[]): number {
    return input.reduce((result, line): number => {
      const game = this.parseLine(line);
      const blues = game.rounds.map(({ blue }) => blue);
      const greens = game.rounds.map(({ green }) => green);
      const reds = game.rounds.map(({ red }) => red);

      const minCubesBlue = Math.max(...blues); 
      const minCubesGreen = Math.max(...greens); 
      const minCubesRed = Math.max(...reds); 

      return result += (minCubesBlue * minCubesGreen * minCubesRed);
    }, 0);
  }

  private parseLine(line: string): Game {
    const [gamePrefix, roundsString] = line.split(':');
    const id = Number.parseInt(gamePrefix.split(' ')[1], 10);
    const rounds = roundsString.split(';').map((roundString): Round => {
      const round: Round = {
        blue: 0,
        green: 0,
        red: 0,
      };
      const entries = roundString.split(',').map((e) => e.trim());

      for (const entry of entries) {
        const [amountString, color] = entry.split(' ');
