...
export class Directory {
  ...
  getSize(): number {
    const subDirectorySize = this.directories.reduce((prev, curr) => prev + curr.getSize(), 0);
    const fileSize = this.files.reduce((prev, curr) => prev + curr.size, 0);

    return subDirectorySize + fileSize;
  }

  toString(offset = 0): string {
    return [
      ' '.repeat(offset) + `"${this.name}": {`,
      ...this.directories.map((dir) => dir.toString(offset + 2)),
      ...this.files.map((file) => ' '.repeat(offset + 2) + `"${file.toString()}",`),
      ' '.repeat(offset) + '},'
    ].join('\n');
  }
...
export type Day7Input = Directory;
 ...
 */
export function solvePart1(input: Day7Input): number {
  let total = 0;
  for (const dir of input.directories) {
    total += solvePart1(dir);
  }

  if (input.getSize() <= 100000) {
    total += input.getSize();
  }
 ...
 * Therefore, the update still requires a directory with total size of 
 ...
 */
export function solvePart2(input: Day7Input): number {
  const minSize = 8381165;
  let foundDirSize = Number.MAX_SAFE_INTEGER;
  for (const dir of input.directories) {
    const smallestOfDir = solvePart2(dir);
    foundDirSize = Math.min(foundDirSize, smallestOfDir);
  }

  const thisSize = input.getSize();
  if (thisSize > minSize) {
    foundDirSize = Math.min(foundDirSize, input.getSize());
  }

  return foundDirSize;
}

export function parseInputFile(filePath: string): Day7Input {
  ...
  for (const line of lines) {
    ...
    switch (parts[0]) {
      case '$':
          ...
          case 'cd':
            switch (parts[2]) {
              ...
              case '/':
                ...
                break;
              default:
                currentDir = currentDir.directories.find((dir) => dir.name === parts[2])!;
                break;
            }
            break;
        }
        break;
      case 'dir':
        currentDir.directories.push(new Directory(parts[1], currentDir));
        break;
      default:
        const [size, name] = parts;
        currentDir.files.push(new File(name, parseInt(size, 10)));
        break;
    }
  }
